...
export class ClaudeSession implements Conversation {
  ...
  async initialize(): Promise<{ threadId: string; model: string }> {
    ...
    this.connection = query({ prompt: input, options: {
      cwd: this.options.cwd, pathToClaudeCodeExecutable: path,
    ...
    this.options.onTelemetry?.({ threadId: this.threadId, model: this.model, provider: 'Claude Code', approval: '等待原生报告' })
  ...
  private receive(message: SDKMessage): void {
    if (message.type === 'system' && message.subtype === 'init') {
      ...
      this.options.onTelemetry?.({ threadId: this.threadId, model: this.model, approval: message.permissionMode, effort: message.effort ?? '默认' })
    }
    ...
    if (message.type === 'assistant') {
      for (const block of message.message.content) {
        ...
        if (block.type === 'tool_use') this.options.onEvent?.({ type: 'tool', text: `${block.name} · ${JSON.stringify(block.input)}` })
      }
  ...
  async command(name: string, argument: string): Promise<string> {
    ...
    if (name === 'usage' || name === 'cost') {
      ...
      const quotas: Quota[] = []
      for (const [key, raw] of Object.entries(usage.rate_limits ?? {})) {
        if (!raw || typeof raw !== 'object' || !('utilization' in raw) || typeof raw.utilization !== 'number') continue
        const reset = 'resets_at' in raw && typeof raw.resets_at === 'string' ? Date.parse(raw.resets_at) / 1000 : null
        quotas.push({ name: key.replace(/_/g, ' '), windows: [{ minutes: key === 'five_hour' ? 300 : key.startsWith('seven_day') ? 10080 : null, usedPercent: raw.utilization, resetsAt: reset !== null && Number.isFinite(reset) ? reset : null }] })
      }
      this.options.onTelemetry?.({ quotas, quotaError: quotas.length ? '' : '原生未提供订阅额度' })
      return `会话估算费用 $${usage.session.total_cost_usd.toFixed(4)}${usage.subscription_type ? ` · ${usage.subscription_type}` : ''}`
    }
