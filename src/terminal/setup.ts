...
export const createSetup = (): SetupState => ({ authSource: 'inherit', view: 'welcome', index: 0, pending: false, status: '连接你的原生 Agent', output: '', input: '', editing: undefined, binaries: { codex: 'codex', claude: 'claude', agy: 'agy' }, members: { codex: undefined, claude: undefined, agy: undefined } })
export function setupActions(state: SetupState): { id: string; label: string }[] {
  ...
  if (state.view === 'welcome') return [
    ...
    { id: 'all', label: '全部成员进入工作区' }, { id: 'use-codex', label: '先使用 Codex' }, { id: 'use-claude', label: '先使用 Claude Code' },
    { id: 'refresh', label: '重新检测' }, { id: 'close', label: '返回工作区 / 退出' },
    { id: 'agy', label: '配置 AGY' }, { id: 'use-agy', label: '使用 AGY' },
  ]
  if (state.view === 'agy') return [
    { id: 'path', label: '设置程序路径' }, { id: 'refresh', label: '检测原生程序' },
    { id: 'use-agy', label: '沿用 AGY 原生登录进入' }, { id: 'back', label: '返回成员选择' },
  ]
  return [
...
export function renderSetup(state: SetupState, columns: number, rows: number, tick: number): { lines: Line[]; cursor: { row: number; column: number }; maxScroll: number } {
  ...
  const tone = state.view === 'welcome' ? 'bright' : state.view
  ...
  body.push(line(state.view === 'welcome' ? '欢迎。先连接一起工作的成员。' : `${memberNames[state.view]}  /  账号与连接`, tone))
  body.push(line('─'.repeat(content)))
  const members = state.view === 'welcome' ? memberIds : [state.view]
  for (const member of members) {
    ...
    body.push(line(`${memberNames[member]}   ${badge}`, member))
    ...
    if (state.view !== 'welcome') {
      ...
      if (info && !info.installed) body.push(...wrap(member === 'agy' ? '请安装原生 agy 并配置可执行文件路径' : member === 'codex' ? '安装：npm install -g @openai/codex' : '安装说明：https://code.claude.com/docs/en/setup', content).map(text => line(text)))
    }
    body.push(line())
  }
