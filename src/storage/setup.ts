...
export interface SetupSettings { authSource?: 'inherit' | 'native'; completed: boolean; selected: MemberId | 'all'; binaries: Record<MemberId, string> }
export class SetupPreferences {
  ...
  async load(): Promise<SetupSettings> {
    try {
      ...
      if (typeof data.completed !== 'boolean' || !['codex', 'claude', 'all'].includes(data.selected) || !valid(data.binaries?.codex) || !valid(data.binaries?.claude)) throw new Error('配置格式无效')
      return { ...(data.authSource === 'native' ? { authSource: 'native' as const } : {}), completed: data.completed, selected: data.selected, binaries: { codex: data.binaries.codex, claude: data.binaries.claude } }
    } catch (error) {
  ...
  async save(settings: SetupSettings): Promise<void> {
    if (!valid(settings.binaries.codex) || !valid(settings.binaries.claude)) throw new Error('程序路径无效')
